...
removeCard index deck =
    case index < (Array.length deck) - 1 of
        True -> Array.append (Array.slice 0 index deck) (Array.slice (index + 1) (Array.length deck) deck)
        False -> deck


evenCardCount : Array Int -> Int
evenCardCount deck =
    Array.length (Array.filter (\x -> remainderBy 2 x == 0) deck)
