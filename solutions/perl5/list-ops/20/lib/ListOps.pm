...
# Tail-recursive for much better performance.
# Omitting the signature avoids this complaint:
# "Use of @_ in list assignment with signatured subroutine is experimental".
sub foldl {
    my ($func, $acc, $list) = @_;
    return $acc unless $#$list >= 0;
    @_ = ($func, $func->( $acc, $list->[0] ), [ $list->@[ 1 .. $#$list ] ]);
    goto __SUB__;
}
