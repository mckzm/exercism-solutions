...
sub is_equilateral ($triple) {
    ...
    return _is_triangle($triple) && $are_equal_sides;
}

sub is_isosceles ($triple) {
    my ( $s1, $s2, $s3 ) = @$triple;
    my $are_two_sides_eq = ( $s1 == $s2 || $s1 == $s3 || $s2 == $s3 );
    return _is_triangle($triple) && $are_two_sides_eq;
}

sub is_scalene ($triple) {
    my ( $s1, $s2, $s3 ) = @$triple;
    my $are_sides_distinct = ( $s1 != $s2 && $s1 != $s3 && $s2 != $s3 );
    return _is_triangle($triple) && $are_sides_distinct;
}

sub _is_triangle($triple) {
    my ( $s1, $s2, $s3 ) = @$triple;
    return
         ( $s1 + $s2 > $s3 )
      && ( $s2 + $s3 > $s1 )
