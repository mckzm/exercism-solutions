defmodule KitchenCalculator do
  ...
  @type unit_tuple :: {atom(), number()}
  @type ml_tuple :: {:milliliter, number()}

  @spec get_volume(unit_tuple()) :: number()
  def get_volume({_, v}), do: v

  for {unit, rate} <- @conversion_map do
    @spec to_milliliter({unquote(unit), number()}) :: {:milliliter, integer()}
    def to_milliliter({unquote(unit), v}), do: {:milliliter, v * unquote(rate)}
  end

  for {unit, rate} <- @conversion_map do
    @spec from_milliliter(ml_tuple(), unquote(unit)) :: {unquote(unit), integer() | float()}
  ...
  @spec convert(unit_tuple(), atom()) :: unit_tuple()
