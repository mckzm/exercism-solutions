...
int count_failed_students(std::vector<int> student_scores) {
    int failed{};
    for (int score: student_scores) if (score <= 40) ++failed;
    return failed;
}
...
std::array<int, 4> letter_grades(int highest_score) {
    ...
    for (int i{0}, interval{(highest_score - 40) / 4}, first_threshold{41}; i < 4; ++i) {
        thresholds.at(i) = first_threshold + (interval * i);
    }
